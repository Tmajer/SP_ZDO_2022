...
from main import InstrumentTracker


def test_run_random():
    dataset_path = os.path.join('resources', 'test')

    print(f'dataset_path = {dataset_path}')
    files = glob.glob(os.path.join(dataset_path, '*.mp4'))
    cislo_souboru = np.random.randint(0, len(files))
    filename = files[cislo_souboru]

    tracker = InstrumentTracker()

    video_name = os.path.split(filename)[1].split('.')[0]
    ...
    if os.path.exists(os.path.join('resources', f'tracking_{video_name}.json')):
        ...
        predictions = tracker.predict(filename)
...
def test_run_all():
    ...
    print(f'dataset_path = {dataset_path}')
    files = glob.glob(os.path.join(dataset_path, '*.mp4'))
    files.extend(glob.glob(os.path.join(dataset_path, '*.mkv')))
    ...
    for filename in files:
        ...
        df_eval = check_one_prediction(filename, predictions)
        f1s.append(df_eval)
